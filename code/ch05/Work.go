...
func ticket(seed int64) (day int) {
	...
	day = int (math.Ceil(float64 (62100000 / (speed * 60 * 60 * 24))))	// 無條件進位
	return
}
...
func main() {
	...
	for i := 0; i < 9; i ++ {
		day := ticket(int64(i))
		TripTypeRand := TripType[rand.Intn(2)]	// 隨機決定單程或往返
		price := rand.Intn(14) +36	// 隨機票價

		if TripTypeRand == "Round - trip" {	// 判斷是否需往返，如果成立則車票錢 * 2
			price *= 2
		}
